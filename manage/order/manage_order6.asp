...
<TR> 
      <TD height="30" align="left" nowrap bgColor=#f4f5fd><font color="#000000"><B><font color="#FF0000">&nbsp;销售统计</font></B></font>&nbsp;&nbsp;&nbsp;&nbsp;<a href="manage_order8.asp">查看销售报表</a></TD>
      </TR>
</TABLE>
...
%>
  <tr bgcolor=#FFFFFF>
	...
	sql1 = "select sum(ProdUnit) from dk501_border A,dk501_borderList B where A.OrderNum = B.OrderNum and B.Status='99' and B.del <> 1"
		if Recname2 <> "" then 
			sql1 = sql1 & " and B.Success_time = '"&Recname2&"'"
		else
			sql1 = sql1 & " and B.Success_time = '"&date()&"'"
		end if
	rs2.open sql1,conn,1,1
	if not (rs2.eof and rs2.bof) then
		Response.Write "销售总件数："&rs2(0)&"&nbsp;件&nbsp;"
	else
		Response.Write "销售总件数：0&nbsp;件&nbsp;"
	end if
	%>
	  <%
	...
	%>
	本页总计利润：<%=formatnumber(total,2)%></td>
  </tr>
  <form name="searchsoft" method="get" action="manage_order6.asp">
  <tr bgcolor="#FFFFFF">
    <td colspan=10 height=30><strong>&nbsp;查找数据：</strong>
      日期：<input name="Recname2" type="text" class=smallInput id="Recname2" size="12" maxlength="12" value="<%if Trim(Request("Recname2"))="" then response.Write date() else response.Write Trim(Request("Recname2")) end if%>">
	  产品编号：<input name="ProdId" type="text" class=smallInput id="ProdId" value="<%=Trim(Request("ProdId"))%>">
