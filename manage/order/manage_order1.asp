...
<TABLE cellSpacing=0 cellPadding=0 width=99% border=0 style="BORDER: #8B979F 1px solid; ">
  <TR>
	...
	%>
      <a href="manage_order1.asp?Status=<%=rsBigClass("Status")%>&show=yes"><%=rsBigClass("StatusDefines")%></a>&nbsp;|&nbsp;
      <%
		rsBigClass.movenext
		loop
	end if 
	...
	%>&nbsp;<b><a href="manage_order_old.asp">2013年1月1日前历史订单</a></b>
    </TD>
  </TR>
</TABLE>
<table height="5">
...
sql="select * from DU_Borderlist_v_2013 where del=0 "
	'开始判断订单状态	
	if Status <> "" then 
...
<table width="99%" border="0" cellpadding="0" cellspacing="1" bgcolor="#8B979F" class="border">
  <tr class="tdbg">
    <form name="searchsoft" method="get" action="manage_order1.asp">
...
if page = 1 then
	...
	response.write "<a href=manage_order1.asp?Recname2="&Recname2&"&Status="&Status&"&page=1>首页</a> <a href=manage_order1.asp?Recname2="&Recname2&"&Status="&Status&"&page="&page-1&">前页</a>"
end if


action="manage_order1.asp?Recname2="&Recname2&"&Status="&Status&""				
 				 if page =< 5 then
							for i=1 to 10								
									if page=i then
...
if page = allpages then
	...
	response.write " <a href=manage_order1.asp?Recname2="&Recname2&"&Status="&Status&"&page="&page+1&">下页</a> <a href=manage_order1.asp?Recname2="&Recname2&"&Status="&Status&"&page="&allpages&">末页</a>"
end if
...
if request.form("edit")="ok" then
	...
	response.write "location.href='manage_order1.asp?action=list&id="&request("ID")&"';"
...
set rs=nothing
	...
	response.write "location.href='manage_order1.asp';"			
	response.write "</script>"
end if
end sub
%>
