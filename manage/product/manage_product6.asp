...
if ProdNum="" or isnull(ProdNum) then
	...
	response.write "location.href='manage_product6.asp?show=yes';"
	response.write "</script>"
...
sql="select * from dk501_product where ProdNum is not null and online=3"
if ProdName <> "" then 
	sql = sql & " and ProdName like '%" & ProdName & "%' "
end if
sql = sql + "order by TJDate desc"
...
%>
<form name=Prodlist action=manage_product6.asp method=post>
  <tr bgcolor="#f4f5fd">
    ...
    <td align=center nowrap>添加/提升日期</td>
    <td colspan="2" align=center nowrap>添加/提升日期</td>
...
%>
  <tr bgcolor="#FFFFFF">
    ...
    <td height="21" align="center"><%=rs("TJDate")%></td>
...
%>
    <td align="center" bgcolor="#F0D0E7"><%response.Write "<img border=0 src="&tjian&" onClick='javascript:location.href=""manage_prod1.asp?action=tuijian&id="&rs("ProdId")&""";'>"%></td>
