...
function do_add(nn){
	$('youfei').value=nn;
	$('sumkiss_text').innerText=eval($('sumkiss').value)+eval(nn);
	return;
	}
</script>
</head>
<body onLoad="do_add(0)">
                    ...
                    <table cellspacing="0" cellpadding="0" width="109%" border="0">
                      <tr>
                        ...
                        <td width="750"><table cellspacing="0" cellpadding="0" width="100%" border="0">
                          ...
                          <input onClick="do_add('<%=Cint(left(totalA,len(totalA)-3))*6+10%>');" type="radio" name="kuaidi" value="申通"/>
                          申通
                          <!--<input onClick="do_add('<%=Cint(left(totalA,len(totalA)-3))*6+10%>');" type="radio" name="kuaidi" value="韵达"/>
                          韵达
                          <input onClick="do_add('<%=Cint(left(totalA,len(totalA)-3))*6+13%>');" type="radio" name="kuaidi" value="圆通" />
                          圆通-->
                          <input onClick="do_add('<%=Cint(left(totalA,len(totalA)-3))*12+22%>');" type="radio" name="kuaidi" value="顺风" />
                          顺风
                          <input onClick="do_add('<%=Cint(left(totalA,len(totalA)-3))*5+10%>');" type="radio" name="kuaidi" value="EMS"  />
                          邮政小包 <span class="800_title"> 邮费： </span>
                          <input name="youfei" type="text" class="buttonface" id="youfei" onBlur="javascript:a();" onKeyUp="value=value.replace(/[^\d]/g,'') " size="6" maxlength="16onbeforepaste=&quot;clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))&quot;" value="<%=Cint(left(totalA,len(totalA)-3))*6+12%>"/>
                          <% else %>
						  <input onClick="do_add('10');" type="radio" name="kuaidi" value="申通"/>
                          申通
                          <!--<input onClick="do_add('10');" type="radio" name="kuaidi" value="韵达"/>
                          韵达
						  <input onClick="do_add('13');" type="radio" name="kuaidi" value="圆通" />
                          圆通-->
                          <input onClick="do_add('22');" type="radio" name="kuaidi" value="顺风" />
                          顺风
                          <input onClick="do_add('12');"type="radio" name="kuaidi" value="EMS" />
                          邮政小包 <span class="800_title"> 邮费： </span>
                          <input name="youfei" type="text" class="buttonface" id="youfei" onBlur="javascript:a();" onKeyUp="value=value.replace(/[^\d]/g,'') " size="6" maxlength="16onbeforepaste=&quot;clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))&quot;" value=12>
                          <% end if %>
                                                         </td>
                                  </tr>
                                  <tr>
                                    ...
                                    <td rowspan="2" align="left" bgcolor="#D9ECC6" class="b-top b-left b-right b-bottom"><input name="alipay" type="radio" value="财富通" checked="checked" />
                                      财富通<span class="800_hzp_red">(4916023 黄曦)</span> <a href="http://www.tenpay.com" target="_blank" class="red">点击进入财付通</a><br />
                                      <input name="alipay" type="radio" value="支付宝" />
                                      支付宝<span class="800_left_red">（13668278656 黄曦）</span><a href="http://www.alipay.com" target="_blank" class="red">点击进入支付宝</a><br />
                                  ...
                                  <tr>
                                    ...
                                    <td height="25" class="800_hzp_red"><input name="Transaction" type="text" id="Transaction" size="30" maxlength="30" />
                                      ...
                                      总应付款：
                                      <span name="sumkiss_text" id="sumkiss_text" style=" color:#F00; font-weight:bold; font-size:18px;"> </span> &nbsp;元</td>
                                  </tr>
                                  <tr>
